...
export const Chat = () => {
  ...
  useEffect(() => {
    let textToType = aiResponse || initialMessage;
  ...
  useEffect(() => {
    if (aiResponse || !aiResponse) { // Handle both initial state and responses
      setIsTyping(true);
      setTypingIndex(0);
      setDisplayText('');
    }
    ...
    const askHandler = async () => {
        ...
        try {
            ...
            const fastApiUrl = process.env.FASTAPI_URL || "http://35.94.209.254:8000";  // Fallback to localhost if not set

            const response = await axios.post(`${fastApiUrl}/agent`, {
